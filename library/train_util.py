...
def add_training_arguments(parser: argparse.ArgumentParser, support_dreambooth: bool):
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="The run name assosciated with the tracker project name",
    )
    parser.add_argument(
        "--log_tracker_config",
        type=str,
    ...
    parser.add_argument(
        ...
        default=None,
        nargs="+",
        help=("A set of prompts to evaluate on and logged to `--report_to`."),
    )
    parser.add_argument(
        "--sample_sampler",
        type=str,
...
def sample_images_common(
    ...
    if distributed_state.num_processes <= 1:
        ...
        with torch.no_grad():
            for prompt_dict in prompts:
                sample_log = sample_image_inference(
                    accelerator,
                    args,
                    pipeline,
                    None,
                ...
                log_commit = log_commit | sample_log
    else:
    ...
    if args.eval_fid:
        ...
        # else:
            ...
            fid_prompts = []
            # preprocess fid prompts
            for i in range(len(args.fid_prompts)):
                dta = args.fid_prompts[i]
                prompt_dict = dta["caption"]
                if isinstance(prompt_dict, str):
                    prompt_dict = line_to_prompt_dict(prompt_dict)
                    fid_prompts.append(prompt_dict)
                assert isinstance(prompt_dict, dict)

                # Adds a name for the file to be saved after. Also cleanup of extra data in original prompt dict.
                prompt_dict["image_id"] = dta["image_id"]
                prompt_dict.pop("subset", None)
            ...
            if distributed_state.num_processes <= 1:
                ...
                with torch.no_grad():
                    for prompt_dict in tqdm(
                        fid_prompts, desc="Generating FID Validation"
                    ):
                ...
                for i in range(distributed_state.num_processes):
                    per_process_prompts.append(
                        fid_prompts[i :: distributed_state.num_processes]
                    )

                num_steps = math.ceil(
                    len(fid_prompts) / distributed_state.num_processes
                )
                progress = tqdm(
        ...
        if False and distributed_state.is_local_main_process:
            ...
            # logger.info("Generating FDD score...")
            # result = subprocess.run(
            #     [
            #         "/home/stanleywu/projects/diffusion-ft/dgm-eval/venv-dgm-3.10/bin/python3",
            #         "-m",
            #         "dgm_eval",
            #         args.test_dir,
            #         validation_dir,
            ...
            # fdd_score = extracted_floats[-1]
            # log_commit["fdd_score"] = fdd_score

            # Run CLIP Aesthetic Score
            ...
            result = subprocess.run(
                [
                    ...
                    validation_dir,
                    "--threshold",
                    str(args.clip_aesthetic_score_threshold),
                ],
                capture_output=True,
                text=True,
            )
...
def sample_image_inference(
    ...
    width = prompt_dict.get("width", args.resolution[0])
    height = prompt_dict.get("height", args.resolution[1])
    ...
    if "image_id" in prompt_dict:
        img_filename = f"{prompt_dict['image_id']}.png"
    else:
    ...
    if save_dir and os.path.exists(os.path.join(save_dir, img_filename)):
        return
