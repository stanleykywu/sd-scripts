...
# Suppress specific FutureWarnings from torch.utils._pytree
...
# Suppress specific FutureWarnings from torch.library.impl_abstract
warnings.filterwarnings(
    "ignore",
...
def train(args):
    ...
    if args.deepspeed:
        ...
        # Assuming 'flux' is your model and 'accelerator.device' is set correctly
        if not is_swapping_blocks:
            flux.to(accelerator.device)  # Move to GPU if not swapping blocks
    ...
    if accelerator.is_main_process:
        ...
        if args.tracker_project_name:
            init_kwargs["wandb"] = {"name": args.tracker_project_name}
        if args.log_tracker_config is not None:
            init_kwargs = toml.load(args.log_tracker_config)
        accelerator.init_trackers(
            "finetuning" if args.log_tracker_config is None else args.log_tracker_name,
...
def setup_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="The json filepath for poison configuration",
    )
    parser.add_argument(
    ...
    #     type=float,
    #     default=None,
    #     help="try 500 steps at a LR of 2e-6 to 4e-6 OR LR of 5e-5 to 1e-4 at up to and above 3-5k steps ",
    # )
    return parser
...
if __name__ == "__main__":
    ...
    poison_config = json.load(open(args.poison_config_fp, "r"))
    args.train_data_dir = poison_config["clean_data_dir"]
    args.in_json = poison_config["clean_data_in_json"]

    args.seed=poison_config["seed"]
    args.pretrained_model_name_or_path = poison_config["pretrained_model_name_or_path"]
    args.clip_l = poison_config["clip_l"]
    args.t5xxl = poison_config["t5xxl"]
    args.ae = poison_config["ae"]
    ...
    args.lr_scheduler=poison_config["lr_scheduler"]
    args.lr_warmup_steps = poison_config["lr_warmup_steps"]
    args.sample_every_n_steps = poison_config["sample_every_n_steps"]
    ...
    args.logging_dir=poison_config.get("logging_dir", None)
    ...
    args.output_dir = os.path.join(
        poison_config["output_dir"], args.tracker_project_name, args.tracker_run_name
    )

    args.clip_aesthetic_score_threshold = poison_config[
    ...
    # args.full_fp16 = poison_config.get("full_fp16", True)
    # args.full_bf16 = poison_config.get("full_bf16", True)
    # args.mixed_precision = "bf16" if args.full_bf16 else "fp16"

# -bash: --max_train_steps: command not found
# -bash: --optimizer_type: command not found
# -bash: --lr_scheduler: command not found
# -bash: --timestep_sampling: command not found
# -bash: --fused_backward_pass: command not found
