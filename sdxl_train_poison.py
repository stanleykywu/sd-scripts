...
def train(args):
        ...
        if args.cache_text_encoder_outputs:
            ...
            with accelerator.autocast():
                train_dataset_group.new_cache_text_encoder_outputs(
                    [text_encoder1, text_encoder2], accelerator.is_main_process
                )
    ...
    # For --sample_at_first
    sample_logs = {}
    sample_logs = sdxl_train_util.sample_images(
        accelerator,
        args,
        0,
        global_step,
    ...
    for epoch in range(num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if args.logging_dir is not None or args.tracker_run_name is not None:
                ...
                combined_logs = logs | sample_logs if sample_logs else logs
                accelerator.log(combined_logs, step=global_step)
                sample_logs = {}
        ...
        if args.logging_dir is not None or args.tracker_run_name is not None:
            ...
            if accelerator.is_main_process:
                accelerator.print(f"Logging {epoch+1} batch loss/epoch: {logs}")
...
def setup_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        nargs="+",
        help=(
            "A set of prompts evaluated every `--validation_epochs` and logged to `--output_dir` and used to compute FID score."
        ),
    )
    parser.add_argument(
        "--test_dir",
    ...
    parser.add_argument(
        ...
        type=float,
        help="Evaluation threshold for clip aesthetic score, only images above this threshold will be considered",
    )
    parser.add_argument(
